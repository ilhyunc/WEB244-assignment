...
function cleanUp(value) {
  let string = value.replace(/([?.!])\s+/g, '$1 ');
...
function createVideo(src, loop, muted) {
  ...
  return `<video src="${src}"${attributes}></video>`;
}
...
function fixPostalCode(postalCode) {
  if (!postalCode) {
    return false; // postalCode가 null 또는 undefined인 경우 처리
  }

  postalCode = postalCode.trim().toUpperCase();

  const pattern = /^[^DFIOQUWZ][0-9][^DFIOQU][\s-]?[0-9][^DFIOQU][0-9]$/;
...
function toProvince(postalCode, useLongForm) {
  ...
  if (postalCode === '' || !/^[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d$/.test(postalCode)) {
    return null;
  }
  ...
  switch (firstLetter.toUpperCase()) {
    ...
    default:
      // Return null for unknown province code
      return null;
  }
}
...
function normalizeCoord(value) {
  ...
  const latLongArray = value.match(regex);

  if (value[0] === '[') {
    [lng, lat] = latLongArray;
  } else {
...
function countForProvince(provinceCode, ...postalCodes) {
  if (postalCodes.length === 0) {
    throw new Error('No postal codes provided');
  }
  ...
  const provCode = provinceMap[provinceCode] || provinceCode;
  let count = 0;
  ...
  for (const postalCode of postalCodes) {
    ...
    const postalCodeProvince = toProvince(postalCode);
    if (postalCodeProvince === provCode) {
      count++;
    }
  }
...
function generateLicenseLink(licenseCode, includeLicenseAttr) {
  ...
  const licenseURL =
    licenseCode && licenseCode.startsWith('CC-')
      ? `https://creativecommons.org/licenses/${licenseCode.slice(3).toLowerCase()}/4.0/`
      : 'https://choosealicense.com/no-permission/';

  const licenseLink = `<a href="${licenseURL}"${includeLicenseAttr ? ' rel="license"' : ''}>${
    licenseText[licenseCode] || 'All Rights Reserved'
  }</a>`;
...
function toBool(value) {
  ...
  if (typeof value === 'number') {
    ...
    let trueVals = ['YES', 'Y', 'OUI', 'O', 'TRUE', 'T', 'VRAI', 'V'];
    let falseVals = ['NO', 'N', 'NON', 'FALSE', 'F', 'FAUX'];
...
function buildQuery(query, perPage, page, format) {
  ...
  if (typeof format !== 'string' || (format !== 'html' && format !== 'json' && format !== 'xml')) {
    throw new Error('invalid format');
  }

  const encodedQuery = encodeURIComponent(query);

  const url = `https://api.web222-example.org/v3/query?query=${encodedQuery}&per_page=${perPage}&page=${page}&format=${format}`;
