...
function observationSummary(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    const { id, species_guess, observed_on_details } = data.results[i];
    const date = observed_on_details.date;

    console.log(`${id}@${date}: ${species_guess}`);
  }
}
...
function observationSummary2(data) {
  // TODO - use forEach()
  data.results.forEach(({ id, species_guess, observed_on_details }) => {
    const date = observed_on_details.date;
...
function observationsByPrivacy(data, geoPrivacy) {
  ...
  if (
    ...
    typeof geoPrivacy === 'number' ||
    typeof geoPrivacy === 'boolean' ||
    geoPrivacy === 'unknown' ||
    geoPrivacy === 'oppen'
  ) {
    throw new Error('Privacy must be a string or null');
  }
  var filteredArray = [];

  for (item of data.results) {
    // if data is geoPrivacy is null then if will be executed
    if (!geoPrivacy && item.geoprivacy === geoPrivacy) filteredArray.push(item);
    else if (
      item.geoprivacy &&
      geoPrivacy &&
      item.geoprivacy.toLowerCase() === geoPrivacy.toLowerCase()
    )
...
function transformObservation(original) {
  const location = original.location.split(',');
  let photoUrls = [];
  for (let i = 0; i < original.photos.length; i++) {
    photoUrls.push(original.photos[i].url);
  }
  ...
  const obj = {
    ...
    geoCoords: [parseFloat(location[1]), parseFloat(location[0])],
    photoUrls: photoUrls,
...
function transformObservations(data) {
  let objs = [];
  for (let i = 0; i < data.results.length; i++) {
    objs.push(transformObservation(data.results[i]));
  }
...
function transformObservations2(data) {
  const objs = data.results.map((result) => transformObservation(result));
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  const { equal, greaterThan, lessThan } = options;

  return data.results.filter((observation) => {
    if (equal !== undefined) {
      return observation.positional_accuracy === equal;
    } else if (greaterThan !== undefined && lessThan !== undefined) {
      return (
        observation.positional_accuracy > greaterThan && observation.positional_accuracy < lessThan
      );
    } else if (greaterThan !== undefined) {
      return observation.positional_accuracy > greaterThan;
    } else if (lessThan !== undefined) {
      return observation.positional_accuracy < lessThan;
    }
...
function getTaxonPhotos(data) {
  ...
  data.results.forEach((result) => {
    if (result.taxon !== undefined && result.taxon.default_photo !== null) {
      const photoID = result.taxon.default_photo.id;
      let observationID = result.id;
      let photo = {
        original:
          'https://static.inaturalist.org/photos/' + photoID + '/original.jpg?' + observationID,
...
function getUserStats(data) {
  ...
  data.results.forEach((result) => {
    totalObservations += result.user.observations_count;
    totalJournals += result.user.journal_posts_count;
